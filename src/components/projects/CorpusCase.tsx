...
const CURRENT_PROJECT_ID = 'Corpus';

interface CorpusCaseProjectProps {
  onBack: () => void;
...
export function CorpusCaseProject({ onBack, onProjectClick }: CorpusCaseProjectProps) {
  ...
  const subtitle = 'Enterprise AI Knowledge Platform';
  const headerColor = '#000000';
  const progressBarColor = '#4F8EF7';
  const arrowColor = '#4F8EF7';
  ...
  const role = 'Lead Product Designer, 0→1';
  const team = <>Hrithik Sanyal, 2 Engineers, 1 PM</>;
  const when = 'March 2024 – May 2024';
  // Speed Read / Gist (2x2 grid in right column)
  const speedReadChallenge = "Placeholder: summarize the core product and user challenge for Corpus.";
  const speedReadProcess = "Placeholder: describe discovery, design iterations, and validation at a high level.";
  const speedReadTakeaways = "Placeholder: note key design decisions and what you would do differently next time.";
  const speedReadImpact = "Placeholder: state measurable or qualitative outcomes once finalized.";
