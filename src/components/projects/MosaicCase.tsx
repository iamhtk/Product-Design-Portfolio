...
const CURRENT_PROJECT_ID = 'Mosaic';

interface MosaicCaseProjectProps {
  onBack: () => void;
...
export function MosaicCaseProject({ onBack, onProjectClick }: MosaicCaseProjectProps) {
  ...
  const subtitle = 'AI Operations Agent Builder';
  const headerColor = '#000000';
  const progressBarColor = '#F7704F';
  const arrowColor = '#F7704F';
  ...
  const when = 'June 2024 – August 2024';
  // Speed Read / Gist (2x2 grid in right column)
  const speedReadChallenge = "Placeholder: frame the operations problem Mosaic set out to solve.";
  const speedReadProcess = "Placeholder: outline how you moved from concept to shipped agent builder.";
  const speedReadTakeaways = "Placeholder: capture lessons on trust, control, and iteration speed.";
  const speedReadImpact = "Placeholder: describe adoption or stakeholder reception once available.";
