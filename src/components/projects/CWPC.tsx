...
interface CWPCProjectProps {
  onBack: () => void;
}

const PROGRESS_BAR_HIDE_DELAY_MS = 400;

export function CWPCProject({ onBack }: CWPCProjectProps) {
  ...
  const title = 'Strudel Design System';
  const company = 'CWPC';
  ...
  const team = 'Hrithik Sanyal, Design Team';
  const when = '2023 - 2024';
  const overview = 'Strudel is a comprehensive design system built to replace the aging Herald Design System. It provides a unified design language, reusable components, and clear documentation to enable teams to build consistent, accessible products faster.';
  const speedReadChallenge = 'Replacing the aging Herald Design System with a modern, scalable foundation. Teams were spending more time fighting the system than building features.';
  const speedReadProcess = 'Research with designers, engineers, and product managers; design principles and token architecture; 50+ production-ready components with accessibility baked in and comprehensive documentation.';
  const speedReadTakeaways = 'Design tokens as the foundation; mobile-first components; governance and contribution workflows for sustainable evolution.';
  ...
  const blocks: ContentBlock[] = [
    { type: 'image', src: '/title-image.mp4' },
    { type: 'text', header: 'THE PROBLEM WITH HERALD', content: 'The Herald Design System had served us well, but as our product suite grew, its limitations became apparent. Components were inconsistent across platforms, documentation was scattered, and design tokens were hardcoded. Teams were spending more time fighting the system than building features. We needed a modern, scalable foundation.' },
    { type: 'text', header: 'CREATING STRUDEL', content: "We started with research—interviewing designers, engineers, and product managers to understand their pain points. The goal was clear: create a system that's flexible enough to support diverse products while maintaining consistency. We established design principles, created a robust token architecture, and built components with accessibility baked in from day one." },
    { type: 'text', header: 'DESIGN TOKENS', content: 'Design tokens are the foundation of Strudel. We created a hierarchical token system covering color, typography, spacing, shadows, and more. These tokens cascade from global values to component-specific overrides, enabling theming and customization while maintaining design integrity. All tokens are platform-agnostic and exported to code automatically.' },
    { type: 'text', header: 'COMPONENT LIBRARY', content: 'We built 50+ production-ready components, each with multiple variants and states. Every component is fully documented with usage guidelines, accessibility notes, and code examples. Components are designed mobile-first and tested across devices. The library includes everything from basic buttons to complex data tables and navigation patterns.' },
    { type: 'text', header: 'DOCUMENTATION & GOVERNANCE', content: 'Good documentation is critical for adoption. We created comprehensive guides covering design principles, component usage, accessibility standards, and contribution workflows. We established a governance model with clear processes for proposing changes, reviewing contributions, and versioning updates. This ensures the system evolves sustainably.' },
    { type: 'text', header: 'IMPACT & ADOPTION', content: 'Six months after launch, Strudel is used across 12 product teams. Development time for new features decreased by 40%. Design-to-development handoff became seamless with shared component libraries. Accessibility compliance improved significantly. Most importantly, our products now feel like they belong to the same family.' },
    { type: 'image', src: 'https://images.unsplash.com/photo-1618761714954-0b8cd0026356?w=1200&q=80' },
  ];
