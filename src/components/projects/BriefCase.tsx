...
const CURRENT_PROJECT_ID = 'Brief';

interface BriefCaseProjectProps {
  onBack: () => void;
...
export function BriefCaseProject({ onBack, onProjectClick }: BriefCaseProjectProps) {
  ...
  const subtitle = 'AI Legal Intelligence Platform';
  const headerColor = '#000000';
  const progressBarColor = '#A64FF7';
  const arrowColor = '#A64FF7';
  ...
  const team = <>Hrithik Sanyal, 1 Engineer, 1 PM</>;
  const when = 'January 2025 – March 2025';
  // Speed Read / Gist (2x2 grid in right column)
  const speedReadChallenge = "Placeholder: define the legal intelligence problem space for Brief.";
  const speedReadProcess = "Placeholder: describe collaboration model and major milestones.";
  const speedReadTakeaways = "Placeholder: note uncertainty handling and auditability themes.";
  const speedReadImpact = "Placeholder: capture impact on review time or risk posture when ready.";
