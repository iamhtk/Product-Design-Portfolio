...
// ReactGA.initialize('G-T6YVTT985T'); // replace with your real Measurement ID
...
//   useEffect(() => {
//     const path = buildPath(currentPage, selectedProjectId);
...
function App() {
  ...
  useEffect(() => {
    const prev = prevNavigationRef.current;
    const isNavigation =
      prev !== null && (prev.page !== currentPage || prev.projectId !== selectedProjectId);
    prevNavigationRef.current = { page: currentPage, projectId: selectedProjectId };
    if (isNavigation) {
      window.scrollTo({ top: 0, left: 0, behavior: 'instant' });
    }
  ...
  // GA4: pageview + special page events + reset timers
  useEffect(() => {
    const path = buildPath(currentPage, selectedProjectId);
    trackPageView(path);
    if (currentPage === 'resume') trackResumeView();
    scrollDepthReached.current = new Set();
    pageEnterTime.current = Date.now();
    return () => {
      const seconds = Math.round((Date.now() - pageEnterTime.current) / 1000);
      if (seconds >= 3) trackTimeOnPage(path, seconds);
    };
  }, [currentPage, selectedProjectId]);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      ([25, 50, 75, 100] as const).forEach((milestone) => {
        if (pct >= milestone && !scrollDepthReached.current.has(milestone)) {
  ...
  // GA4: outbound link + email tracking
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      const target = (e.target as HTMLElement).closest('a');
      if (!target) return;
      const href = target.getAttribute('href') ?? '';
      const isOutbound = href.startsWith('http') && !href.includes('hrithiksanyal.com');
      const isMailto = href.startsWith('mailto:');
      if (isOutbound) {
        const label = target.textContent?.trim() || target.getAttribute('aria-label') || href;
        import('./services/analytics').then(({ trackOutboundLink }) =>
          trackOutboundLink(label, href)
        );
      }
      if (isMailto) {
        import('./services/analytics').then(({ trackEmailClick }) => trackEmailClick());
      }
    };
